...
Whatever address this *A-register* hold, is the address of the register, which value the RAM will be emitting at that moment.

@19
1. The value that is stored in the *A-register* will be 19
2. The value that is emitted by the RAM will be the value of the register stored with address 19
3. The register that with address 19 becomes available to be manipulated
